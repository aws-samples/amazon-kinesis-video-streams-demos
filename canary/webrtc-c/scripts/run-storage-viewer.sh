...
if [ -n "${JS_PAGE_URL:-}" ]; then
    if [[ "${JS_PAGE_URL}" != *"://"* ]]; then
        JS_SDK_DIR="${VIEWER_BUILD_HOME}/${JS_PAGE_URL}/repo"
        LOGS_DIR="${VIEWER_BUILD_HOME}/${JS_PAGE_URL}/logs"
        mkdir -p "$LOGS_DIR"
        ...
        ls -1t "$LOGS_DIR"/dev-server-*.log 2>/dev/null | tail -n +11 | xargs rm -f 2>/dev/null || true
