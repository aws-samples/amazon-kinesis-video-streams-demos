...
def buildStorageCanary(isConsumer, params) {
    ...
    // Continuous/soak runs: the consumer keeps its metric timers alive and runs until killed
    // instead of exiting after CANARY_DURATION_IN_SECONDS. Pairs with CONSUMER_AUTO_REFRESH_CREDS
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'CANARY_MEDIA_SOURCE', defaultValue: '', description: 'Master media source: empty/disk=pre-encoded frames (no GStreamer), framesrc=the same pre-encoded frames re-encoded live through GStreamer and looped forever (USE THIS FOR A SOAK: it is the only non-disk source that both loops and carries the burned-in frame counter SSIM verification needs), devicesrc=physical camera, testsrc=GStreamer test pattern, rtspsrc=RTSP, filesrc=local video file (ends at EOF -- unsuitable for a soak, see GstMedia.cpp:547). Non-disk values trigger a GStreamer-enabled build (requires libgstreamer1.0-dev on the master node).')
    ...
    stages {
        ...
        stage('Build and Run Storage Master and Consumer on Same Node') {
            ...
            parallel {
                ...
                stage('SingleNodeStorageConsumer') {
                    steps {
                        script {
                            buildStorageCanary(true, params)
                        }
